...
export async function renderCard(layoutJson: string, studentData: any): Promise<HTMLCanvasElement> {
...
export async function renderA4Sheet(
  ...
  const card1 = await renderCard(layoutJson, student1)
  const card2 = student2 ? await renderCard(layoutJson, student2) : null
