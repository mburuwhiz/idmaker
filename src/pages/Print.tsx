...
const Print: React.FC<PrintProps> = ({ initialBatchId = null }) => {
  ...
  const updatePreview = async () => {
    ...
    if (viewMode === 'pair') {
      ...
      const getPData = async (student: any) => {
        if (!student || !student.photoPath) return undefined
      ...
      const p1Data = await getPData(student1)
      const p2Data = await getPData(student2)
      ...
      const pData = await getPData(student)
  ...
  const handlePrintAll = async () => {
    ...
    try {
      ...
      for (let i = 0; i < Math.ceil(students.length / 2); i++) {
        ...
        const p1Data = await getPData(s1)
        const p2Data = await getPData(s2)
